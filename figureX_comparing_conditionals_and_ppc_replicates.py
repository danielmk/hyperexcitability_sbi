...
f = os.path.join(Metadata.results_dir,
                 "truncated_sequential_npe_network_baseline_conductance_based_01_baseline_replicate_01.pickle")

output_path = os.path.join(Metadata.results_dir, 'conditionals_output_data_replicate_01.h5')

data_file = tables.open_file(output_path, mode='r')

x_in_loss_healthy = data_file.root.in_loss.x_healthy.read()

x_in_loss_in_loss = data_file.root.in_loss.x_unhealthy.read()

x_hyperexcitable_healthy = data_file.root.hyperexcitable.x_healthy.read()

x_hyperexcitable_hyperexcitable = data_file.root.hyperexcitable.x_unhealthy.read()

x_sprouting_healthy = data_file.root.sprouting.x_healthy.read()

x_sprouting_sprouting = data_file.root.sprouting.x_unhealthy.read()
...
ks_test_hyperexcitable = data_file.root.hyperexcitable.ks_test.read()
...
ks_test_sprouting = data_file.root.sprouting.ks_test.read()

ks_test_sprouting = np.insert(ks_test_sprouting, 14, [np.nan, np.nan], axis=0)

colors = ['#66c2a5', '#fc8d62', '#8da0cb']

correlations_path = os.path.join(Metadata.results_dir, 'marginal_and_conditional_correlation_matrices_replicate_01.h5')
...
theta_in_loss_in_loss = data_file.root.in_loss.theta_unhealthy.read()

theta_he_healthy = data_file.root.hyperexcitable.theta_healthy.read()

theta_he_unhealthy = data_file.root.hyperexcitable.theta_unhealthy.read()

theta_sprouting_healthy = data_file.root.sprouting.theta_healthy.read()

theta_sprouting_sprouting = data_file.root.sprouting.theta_unhealthy.read()
